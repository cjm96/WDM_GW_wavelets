...
from ._time_shift_jax import (
    assemble_shift_target_batch_chunked_lagblock_edge_strip_jax,
    assemble_shift_target_batch_chunked_lagblock_interior_jax,
...
class VariableShiftBatchPlan:
    ...
    def apply_device_interior_fast(
        self,
        coefficients,
        *,
        return_profile: bool = False,
        ...
        """Apply the experimental interior-row/source-halo fast path.

        The normal :meth:`apply_device` method remains unchanged. This method
        is intentionally separate so the candidate can be validated and timed
        against the production lag-blocked route using the same prepared plan.
        """
        ...
        if not self.resolved_use_jax:
            raise NotImplementedError(
                "The interior fast-path experiment requires a JAX-backed plan."
            )
        if self.resolved_assembly_backend != "lagfirst_chunked_lagblock":
            raise NotImplementedError(
                "The interior fast-path experiment currently supports only "
        ...
        row_chunk_size = int(self.config.row_chunk_size)
        n_chunks = (self.Nt + row_chunk_size - 1) // row_chunk_size
        interior_chunk_count = sum(
            1
            for chunk_id in range(n_chunks)
            if (
                chunk_id * row_chunk_size >= self.offset
                and (chunk_id + 1) * row_chunk_size
                <= self.Nt - self.offset
            )
        )
        ...
        profile: dict[str, float | int | str | bool] = {
            ...
            "interior_fast_path": True,
            "interior_chunk_count": int(interior_chunk_count),
            "total_row_chunk_count": int(n_chunks),
            "persistent_memory_increase_bytes": 0,
            "returned_on_device": True,
        }
        return result_device, profile

    def apply_device_interior_edge_strip(
        self,
        coefficients,
        *,
        return_profile: bool = False,
        ...
        """Apply the exact edge-strip interior/source-halo fast path.
        ...
        if not self.resolved_use_jax:
            raise NotImplementedError(
                "The edge-strip experiment requires a JAX-backed plan."
            )
        if self.resolved_assembly_backend != "lagfirst_chunked_lagblock":
            raise NotImplementedError(
                "The edge-strip experiment currently supports only "
                "assembly_backend='lagfirst_chunked_lagblock'; got "
                f"{self.resolved_assembly_backend!r}."
            )
        ...
        row_chunk_size = int(self.config.row_chunk_size)
        interior_row_count = max(0, self.Nt - 2 * self.offset)
        full_interior_chunk_count = (
            interior_row_count // row_chunk_size
        )
        interior_remainder_rows = (
            interior_row_count % row_chunk_size
        )
        ...
        profile: dict[str, float | int | str | bool] = {
            ...
            "device_plan_cached": bool(cache_device_plan),
            "device_plan_memory_bytes": int(self.device_plan_memory_bytes),
            "interior_edge_strip": True,
            "interior_row_count": int(interior_row_count),
            "boundary_row_count": int(self.Nt - interior_row_count),
            "full_interior_chunk_count": int(full_interior_chunk_count),
            "interior_remainder_rows": int(interior_remainder_rows),
            "persistent_memory_increase_bytes": 0,
            "returned_on_device": True,
        }
        return result_device, profile
