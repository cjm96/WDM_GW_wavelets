...
The public row-chunked lag-blocked target backend uses an exact analytic-parity
reduction of the deterministic WDM checkerboard. The previous checkerboard
implementation is retained through explicit ``*_reference_jax`` wrappers.
"""
...
def assemble_shift_target_chunked_lagblock_reference_jax(
    w_xi,
    t_shift,
    ell_all,
    offset,
...
def assemble_shift_target_batch_chunked_lagblock_reference_jax(
    w_xi_batch,
    t_shift_batch,
    ell_all,
    offset,
...
# ---------------------------------------------------------------------------
# Production analytic-parity lag-blocked backend
# ---------------------------------------------------------------------------
#
# The WDM checkerboard factors Cnm and the accompanying carrier/sideband
# parity phases are deterministic.  Their exact products reduce to compact
# signs depending only on target-row parity, lag parity/modulo four, and
# frequency parity.  Replacing the large Cnm gathers with these signs reduced
# the full six-link c64 shift runtime from 0.648 s to 0.573 s (11.5%) in the
# validated full-year benchmark, with relative L2 difference 5.52e-08.
#
# The previous implementation remains available through the explicit
# ``*_reference_jax`` wrappers above.  The normal public lag-block wrappers
# below now use the analytic-parity kernels while retaining the existing API;
# ``Cnm`` is accepted but intentionally unused for compatibility.


def _analytic_parity_pm_one(values, real_dtype):
    """Return ``(-1)**values`` in the requested real dtype."""
...
def _make_shift_target_chunked_lagblock_analytic_parity_impl(
    *,
    ...
    """Create one compact analytic-parity lag-blocked shift kernel."""
    ...
    def impl(
        ...
        # Build all compact parity data once outside the row/lag loops.
        ...
        def chunk_body(chunk_id, out_acc):
            ...
            def lag_block_body(lag_block_id, chunk_out):
                ...
                # Exact carrier checkerboard product:
                ...
                def add_sidebands(main_acc):
                    # Exact sideband checkerboard products are +/-i times
                    # compact row/lag/frequency signs.
                    even_row_lag_sign = (
                        row_sign[:, None]
                        * even_half_sign[None, :]
                    )
...
_assemble_shift_target_chunked_lagblock_analytic_parity_impl_c128 = (
    _make_shift_target_chunked_lagblock_analytic_parity_impl(
        complex_dtype=jnp.complex128,
        real_dtype=jnp.float64,
    )
)
_assemble_shift_target_chunked_lagblock_analytic_parity_impl_c64 = (
    _make_shift_target_chunked_lagblock_analytic_parity_impl(
        complex_dtype=jnp.complex64,
        real_dtype=jnp.float32,
    )
...
_assemble_shift_target_chunked_lagblock_analytic_parity_core_c128 = jax.jit(
    _assemble_shift_target_chunked_lagblock_analytic_parity_impl_c128,
    static_argnums=(3, 7, 8),
)
_assemble_shift_target_chunked_lagblock_analytic_parity_core_c64 = jax.jit(
    _assemble_shift_target_chunked_lagblock_analytic_parity_impl_c64,
    static_argnums=(3, 7, 8),
)


def _make_shift_target_batch_chunked_lagblock_analytic_parity_impl(
    *,
    single_job_impl,
    complex_dtype,
):
    """Create the batched compact analytic-parity kernel."""
...
_assemble_shift_target_batch_chunked_lagblock_analytic_parity_impl_c128 = (
    _make_shift_target_batch_chunked_lagblock_analytic_parity_impl(
        single_job_impl=(
            _assemble_shift_target_chunked_lagblock_analytic_parity_impl_c128
        ),
        complex_dtype=jnp.complex128,
...
_assemble_shift_target_batch_chunked_lagblock_analytic_parity_impl_c64 = (
    _make_shift_target_batch_chunked_lagblock_analytic_parity_impl(
        single_job_impl=(
            _assemble_shift_target_chunked_lagblock_analytic_parity_impl_c64
        ),
...
_assemble_shift_target_batch_chunked_lagblock_analytic_parity_core_c128 = jax.jit(
    _assemble_shift_target_batch_chunked_lagblock_analytic_parity_impl_c128,
    static_argnums=(3, 7, 8),
)
_assemble_shift_target_batch_chunked_lagblock_analytic_parity_core_c64 = jax.jit(
    _assemble_shift_target_batch_chunked_lagblock_analytic_parity_impl_c64,
    static_argnums=(3, 7, 8),
)
...
def assemble_shift_target_chunked_lagblock_jax(
    ...
    """Assemble one target-mode shift with analytic checkerboard parity.

    ``Cnm`` is retained in the signature for backward compatibility but is
    not read by this optimized backend.
    """
    ...
    if precision == "complex64":
        out = _assemble_shift_target_chunked_lagblock_analytic_parity_core_c64(
            jnp.asarray(w_xi, dtype=jnp.complex64),
            jnp.asarray(t_shift, dtype=jnp.float32),
            jnp.asarray(ell_all, dtype=jnp.int64),
            int(offset),
    ...
    out = _assemble_shift_target_chunked_lagblock_analytic_parity_core_c128(
        jnp.asarray(w_xi, dtype=jnp.complex128),
        jnp.asarray(t_shift, dtype=jnp.float64),
        jnp.asarray(ell_all, dtype=jnp.int64),
        int(offset),
...
def assemble_shift_target_batch_chunked_lagblock_jax(
    ...
    """Assemble batched target-mode shifts with analytic parity.
    ...
    if precision == "complex64":
        out = (
            _assemble_shift_target_batch_chunked_lagblock_analytic_parity_core_c64(
                jnp.asarray(w_xi_batch, dtype=jnp.complex64),
                jnp.asarray(t_shift_batch, dtype=jnp.float32),
                jnp.asarray(ell_all, dtype=jnp.int64),
                int(offset),
    ...
    out = _assemble_shift_target_batch_chunked_lagblock_analytic_parity_core_c128(
        jnp.asarray(w_xi_batch, dtype=jnp.complex128),
        jnp.asarray(t_shift_batch, dtype=jnp.float64),
        jnp.asarray(ell_all, dtype=jnp.int64),
        int(offset),
